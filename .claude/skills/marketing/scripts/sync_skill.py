#!/usr/bin/env python3
"""Dong bo skills/marketing (nguon chuan) sang .agents/skills/marketing va
.claude/skills/marketing (ban sao cho Codex/Claude Code).
...
SOURCE_DIR = SCRIPT_DIR.parent  # skills/marketing
REPO_ROOT = SOURCE_DIR.parent.parent  # thu muc goc repo

TARGETS = [
    REPO_ROOT / ".agents" / "skills" / "marketing",
    REPO_ROOT / ".claude" / "skills" / "marketing",
]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="sync_skill.py",
        description=(
            "Dong bo skills/marketing (nguon chuan duy nhat) sang "
            ".agents/skills/marketing va .claude/skills/marketing."
        ),
    )
...
def safe_target_check(target: Path) -> None:
    expected_suffix = Path("skills") / "marketing"
...
def do_check() -> int:
    ...
    all_synced = True
    labels = [".agents/skills/marketing", ".claude/skills/marketing"]
